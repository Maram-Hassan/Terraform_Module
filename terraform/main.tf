...
module "frontend" {
  ...
  subnet_id     = module.network.public_subnet_id  # Public subnet for frontend
...
module "backend" {
  ...
  ami_id        = "ami-0866a3c8686eaeeba"  # Update with the Ubuntu 22.04 AMI ID
  instance_type = "t3.micro"
  subnet_id     =  module.network.public_subnet_id  # Private subnet for backend
  # vpc_id        = module.network.aws_vpc.myvpc.id
...
module "rds" {
  ...
  db_subnet_group   = module.network.private_subnet_id # Private subnet for RDS
